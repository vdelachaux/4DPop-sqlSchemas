//%attributes = {"invisible":true,"shared":true,"publishedSql":true}
// METHOD_NAME : SC_Schema_Get_ID
// Module : SC
// Username : jpupier
// Created : 01/10/08, 14:19:14
// ============================================================================
// Description
// Returns the unique ID for a given schema name
//
// Parameters
C_TEXT:C284($1; SC_private_SchemaName_t)
SC_private_SchemaName_t:=$1
C_LONGINT:C283($0; SC_private_SchemaID_l)
SC_private_SchemaID_l:=0
// ----------------------------------------------------
// Process_Variables
//
// ----------------------------------------------------
// Local_Variables
//
// ----------------------------------------------------
// Local_Init
//
// ----------------------------------------------------
// ============================================================================

If (SC_Schemas_Supported)
	
	Begin SQL
		select SCHEMA_ID from [_USER_SCHEMAS] where SCHEMA_NAME = :SC_private_SchemaName_t into :SC_private_SchemaID_l;
	End SQL
	
End if 


// ============================================================================
$0:=SC_private_SchemaID_l
//eof




