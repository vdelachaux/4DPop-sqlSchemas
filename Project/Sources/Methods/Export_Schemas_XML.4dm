...
For ($Loop_l; 1; Size of array:C274($SchemaName_at_p->))
	...
	If ($SchemaName_at_p->{$Loop_l}#SC_K_Default)
		...
		$Ref_t:=DOM Create XML element:C865($Root_t; $Path_t; "name"; $SchemaName_at_p->{$Loop_l}; "read"; $Read_at_p->{$Loop_l}; "read_write"; $ReadWrite_at_p->{$Loop_l}; "all"; $All_at_p->{$Loop_l})
	End if 
